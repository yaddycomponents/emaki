...
import { type AnimSpec, type Channel } from '../presets'
...
/**
 * The style of an AnimSpec at a normalized progress (0→1), eased. Used by the
 * state motion layer (enter/exit + camera) to interpolate without frame math.
 */
export function sampleSpec(spec: AnimSpec, progress: number): RenderStyle {
  const eased = cubicBezier(spec.ease)(clamp(progress, 0, 1))
