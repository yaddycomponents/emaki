...
const UI_NODE_DOCS: Record<string, string> = {
  ...
  sparkle: "the animated AI mark — a pink→blue gradient star (size, color?)",
  vector: "a custom mark from SVG path data (paths:[{d,fill?,stroke?,strokeWidth?}], viewBox, w, h) — the open visual escape hatch",
  toggle: "switch (on)",
...
function uiNodeReference(): string {
  ...
  return [
    ...
    "AI look: `sparkle` node, `text`/`dot` `gradient:\"ai\"` (pink→blue), and `badge` tone:\"ai\" (sparkle + gradient pill).",
    ...
    "  · any node's `anim`: a preset name OR inline `{ from:{x?,y?,opacity?,scale?,rotate?}, dur?, ease? }` — author any entrance motion.",
    "  · any container is a `box`: `bg`/`border`/`radius`/`shadow` (bg/border take a theme token, a hex, or \"ai\").",
    "  · `vector`: draw any custom mark from SVG path data. Use these instead of asking for a new primitive.",
    "",
