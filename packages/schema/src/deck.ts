...
function sceneEnvelope<T extends string, P extends z.ZodType>(
  type: T,
  props: P,
) {
  return z.object({
  ...
  return z
    .object({
      ...
      title: z
        .string()
        .optional()
        .meta({ description: "Human label for the deck." }),
      ...
      wordsPerSecond: z
        ...
        .meta({
          description: "Narration pace used to compute scene durations.",
        }),
    ...
    .meta({
      ...
      title: "Emaki Deck",
      description:
        "A deck is an ordered list of scenes rendered at one aspect ratio.",
    });
}
...
export function buildDeckSchema(blocks: readonly BlockDef[]) {
  ...
  if (members.length === 0)
    throw new Error("buildDeckSchema needs at least one block.");
