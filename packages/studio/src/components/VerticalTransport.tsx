import { ChevronLeft, ChevronRight, Play, Pause, Repeat, Maximize2 } from 'lucide-react'
import { useStudio } from '../store'
import { sceneFrames, timecode } from '../timing'
import s from './VerticalTransport.module.css'

export function VerticalTransport() {
  ...
  const start = frames.slice(0, selected).reduce((a, b) => a + b, 0)
  ...
  return (
    <div className={s.bar}>
      <div className={s.cluster}>
        ...
        <button type="button" className={playing ? `${s.btn} ${s.playOn}` : s.btn} onClick={togglePlay} aria-label="play/pause">
      ...
      <div className={s.time}>
        ...
        <span className={s.timeSep}>/ {timecode(total)}</span>
      </div>
